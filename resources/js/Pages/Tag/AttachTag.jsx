...
export default function AttachTag({props, postId, tagList}) {
		...
		<section className="absolute inset-0 top-0 left-0 right-10 bottom-0 h-full overflow-hidden w-screen z-40 bg-gray-500/50">
			<div className="relative flex mx-auto py-10">
				...
				<div className="flex w-auto h-auto mx-auto p-10">
					...
					<h2 className="text-center text-2xl text-zinc-500 border-b border-slate-300 my-3 font-bold">Tags for Post ID: {postId}</h2>
					...
					<div className="py-3">
						...
						<div className="relative mx-auto text-center pb-4">
							...
							<Link onClick={(e) => submit(e)}  disabled={processing} className="transition delay-50 px-4 py-2 text-md- font-medium bg-sky-500 hover:bg-sky-600 text-slate-50 rounded-r-sm" type="submit">Attach</Link>
						</div>
					</div>
				</form>
				</div>
