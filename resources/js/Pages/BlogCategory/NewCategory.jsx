...
export default function NewCategory({props, auth}) {
		...
		<GeneralLayout auth={auth}>
			...
			<section className="relative flex h-screen overflow-hidden mx-auto p-5 items-center">
				<form onSubmit={submit} className="bg-white drop-shadow-xl w-96 rounded-sm px-10 py-4">
					<h2 className="text-center text-2xl text-zinc-500 border-b border-slate-300 my-3 font-bold">New Blog Category</h2>
					<div className="py-3">
						<div className="mb-3">
							<label htmlFor="blog_category_title" className="border-b border-gray-300 text-md text-gray-700 font-bold block my-2.5">Title <span className="text-red-500">*</span></label>
							<input 
								id="blog_category_title" 
								type="text" 
								value={data.blog_category_title} 
								onChange={e => setData('blog_category_title', e.target.value)} 
								className="transition delay-50 w-full appearance-none active:text-slate-50 border-none rounded-sm focus:border-1 focus:bg-slate-200 focus:text-slate-600 focus:border-gray-700 ring-none bg-slate-500 text-slate-900 text-md placeholder-slate-50" 
						...
						<div className="mb-3">
							<label htmlFor="blog_category_description" className="border-b border-gray-300 text-md text-gray-700 font-bold block my-2.5">Description</label>
							<textarea 
