...
export default function NewProjectCategory({props, visibility}) {
	const [visible, setVisibility] = useState({visibility})
	const {data, setData, post, processing, errors} = useForm({
		project_category_title: '',
		project_category_description: '',
		project_category_enabled: true,
	})
	
	function submit(e) {
		e.preventDefault()
		
		post(route('projectCategory.store'))
		...
		<section className="absolute inset-0 top-0 left-0 right-10 bottom-0 h-full overflow-hidden w-screen z-40 bg-gray-500/50">
			<div className="relative flex mx-auto py-10">
				<div className="flex w-auto h-auto mx-auto p-10">
					...
					<h2 className="text-center text-2xl text-zinc-500 border-b border-slate-300 my-3 font-bold">New Project Category</h2>
					<div className="py-3">
						<div className="mb-3">
							<label htmlFor="project_category_title" className="border-b border-gray-300 text-md text-gray-700 font-bold block my-2.5">Title <span className="text-red-500">*</span></label>
							<input 
						...
						<div className="mb-3">
							<label htmlFor="project_category_description" className="border-b border-gray-300 text-md text-gray-700 font-bold block my-2.5">Description</label>
							<textarea 
